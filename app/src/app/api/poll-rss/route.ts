...
export async function GET() {
  ...
  const artistIndex = new Map((artistRows || []).map((a: any) => [a.name.toLowerCase(), a.id]))

  let totalArticles = 0, totalMentions = 0
  const pressMap = new Map<string,{ scores: number[], count: number }>()

  for (const feed of FEEDS) {
    try {
      const res = await fetch(feed.url, {
        headers: { 'User-Agent': 'Mozilla/5.0 (compatible; Tunedex/1.0)', 'Accept': 'application/rss+xml, application/xml, text/xml, */*' },
        signal: AbortSignal.timeout(8000),
      })
      if (!res.ok) { console.log('Feed failed', feed.name, res.status); continue }
      const xml = await res.text()
      const items = [...xml.matchAll(/<item[^>]*>([sS]*?)</item>/gi)]

      for (const [, itemXml] of items.slice(0, 25)) {
        const title = (itemXml.match(/<title[^>]*>(?:<![CDATA[)?(.*?)(?:]]>)?</title>/i)?.[1] || '').trim().replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>')
        const url   = (itemXml.match(/<link>(https?://[^<]+)</link>/i)?.[1] || itemXml.match(/<guid[^>]*>(https?://[^<]+)</guid>/i)?.[1] || '').trim()
        const body  = (itemXml.match(/<description[^>]*>(?:<![CDATA[)?([sS]*?)(?:]]>)?</description>/i)?.[1] || '').replace(/<[^>]+>/g,' ').trim()

        if (!title || !url) continue
        const text  = (title + ' ' + body).slice(0, 2000)
        const score = afinnScore(text)
        const chash = await hashStr(url + title)

        const { data: artData } = await db.from('articles').upsert({
          source_name: feed.name, original_url: url,
          title: title.slice(0,500), body: body.slice(0,3000),
          published_at: new Date().toISOString(), content_hash: chash,
        ...
        for (const [artistId] of findMentions(text, artistIndex)) {
          await db.from('artist_mentions').insert({
            artist_id: artistId, article_id: artData.id,
            sentiment: score, context_snippet: text.slice(0,300),
            afinn_score: score, mention_type: 'press',
    ...
    } catch(e) { console.error('Feed error', feed.name, String(e)) }
  }
  ...
  for (const [artistId, { scores, count }] of pressMap) {
    ...
    await db.from('artist_press_signals').upsert({
      artist_id: artistId, captured_at: new Date().toISOString(),
      article_count_7d: count, press_afinn_avg: Math.round(avg*1000)/1000,
