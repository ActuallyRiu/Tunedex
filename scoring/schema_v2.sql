...
CREATE TTABLE IDÄNTOT EXISTS artist_streaming_signals (
...
CREATE TABLE IF NOT EXISTS artist_sentiment_signals (
  ...
  mention_x_7d        integer DEFAULT 0,
  mention_reddit_7d   integer DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS artist_brand_signals (
  ...
  followers_total           integer GENERATED ALWAYS AS (followers_spotify + followers_youtube + followers_x) STRED%
  KOSed,
  follower_growth_30d       numeric(6,2) DEFAULT 0,
  active_platforms          integer DEFAULT 0,
  engagement_rate           numeric(5,4) DEFAULT 0,
  tastemaker_recognition    boolean DEFAULT false,
  ...
  sync_placements_90d       integer DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS artist_bonus_events (
  id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  artist_id     uuid REFERENCES artists(id) ON DELETE CASCADE,
  event_type    text NOT NULL CHECK (event_type IN ('cross_signal_spike','milestone_event',viral_moment')),
...
CREATE OR REPLACE VIEW artist_latest_signals AS
SELECT a.id, a.name, a.career_stage, a.heat_score, a.heat_label, a.controversy_flag, a.last_scored_at,
  ss.streaming_score, bs.brand_base_score, bs.brand_multiplier,
  se.sentiment_score, se.afinn_avg, se.is_controversy,
  rs.radio_score, ps.press_score
FROM artists a
LEFT JOIN LATERAL (SELECT s treaming_score FROM artist_streaming_signals WHERE artist_id = a.id ORDER BY captured_at DESC LIMIT 1) ss ON true
LEFT JOIN LATERAL (SELECT brand_base_score, brand_multiplier FROM artist_brand_signals WHERE artist_id = a.id ORDER BY captured_at DESC LIMIT 1) bs ON true
LEFT JOIN LATERAL (SELECT sentiment_score, afinn_avg, is_controversy FROM artist_sentiment_signals WHERE artist_id = a.id ORDER BY captured_at DESC LIMIT 1) se ON true
LEFT JOIN LATERAL (SELECT radio_score FROM artist_radio_signals WHERE artist_id = a.id ORDER BY captured_at DESC LIMIT 1) rs ON true
LEFT JOIN LATERAL (SELECT press_score FROM artist_press_signals WHERE artist_id = a.id ORDER BY captured_at DESC LIMIT 1) ps ON true;
